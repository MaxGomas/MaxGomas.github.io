...
function init(){
    window.requestAnimFrame = (function() {
      ...
      function draw() {
        ...
        var t = "ColorBall".split("").join(String.fromCharCode(0x2004));
        $.font = "5em Poiret One";
        $.fillStyle = 'hsla(213, 95%, 15%, 1)';
        $.fillText(t, c.width / 2 - 280, c.height / 2);
        var t2 = "Faites Glisser Pour Dessiner";
        $.font = "2em Poiret One";
        $.fillStyle = 'hsla(213, 95%, 15%, 1)';
        $.fillText(t2, c.width / 2 - 180, c.height / 2 + 100);
        balls.fr(balls, $, planks);
        planks.draw(planks, $);
        myReq = window.requestAnimFrame(draw);
      }
...
function launch(){
  ...
  var btn = document.getElementById("abs").style.visibility = "hidden";
